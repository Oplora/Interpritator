...
public:
    Scanner (const char * program) : count(1) {
        fp = fopen(program, "r");
        cout<<"WAS HERE"<<endl<<program<<endl;
        if (fp == NULL) err(0);
        clear();
        gc();
    }
...
//--------------------IDENTITY, STACK AND TABLE CLASS--------------------//
...
class Parser {
    ...
    void gl () {
        lex = scan.get_lex();
        if (z1) lex.print(); // Отладка лексического
    }
...
int main(int argc, char **argv) {
    ...
    try {
        cout<< argc<< endl;
        char cur_dir[strlen(argv[0])] = "/home/oplora/Desktop/Inter/additional/";
